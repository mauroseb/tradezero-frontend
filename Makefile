...
push: ## Push container to defined registry
	podman push ${REGISTRY}/${REG_NAMESPACE}/${NAME}:${VERSION}

.PHONY: clean
clean: ## Clean container images locally
	podman rmi ${REGISTRY}/${REG_NAMESPACE}/${NAME}:${VERSION} 2> /dev/null || true
	podman rm -f tradezero-frontend-1 2> /dev/null || true
